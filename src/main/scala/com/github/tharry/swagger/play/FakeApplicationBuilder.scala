...
import play.api.inject.guice.{BinderOption, GuiceBuilder, GuiceableModule}
...
class FakeApplicationBuilder(
                              ...
                              loadModules: (Environment, Configuration) => Seq[GuiceableModule] = GuiceableModule.loadModules) extends GuiceBuilder[FakeApplicationBuilder](
   ...
   */
  def loadConfig(loader: Environment => Configuration): FakeApplicationBuilder =
   ...
   * Overrides the default or any previously configured values.
   */
  def loadConfig(conf: Configuration): FakeApplicationBuilder =
  loadConfig(env => conf)
   ...
   * Overrides the default or any previously configured values.
   */
  def load(loader: (Environment, Configuration) => Seq[GuiceableModule]): FakeApplicationBuilder =
  copy(loadModules = loader)

  /**
   * Override the module loader with the given modules.
   */
  def load(modules: GuiceableModule*): FakeApplicationBuilder =
   ...
   */
  def appRoutes(routes: Application => PartialFunction[(String, String), Handler]): FakeApplicationBuilder =
  ...
  def routes(routesFunc: PartialFunction[(String, String), Handler]): FakeApplicationBuilder =
  appRoutes(_ => routesFunc)

  /**
   * Override the router with the given router.
   */
  def router(router: Router): FakeApplicationBuilder =
   ...
   */
  def additionalRouter(router: Router): FakeApplicationBuilder =
  ...
  loadModules: (Environment, Configuration) => Seq[GuiceableModule] = loadModules): FakeApplicationBuilder =
  new FakeApplicationBuilder(environment, configuration, modules, overrides, disabled, binderOptions, eagerly, loadConfiguration, loadModules)
  ...
  binderOptions: Set[BinderOption] = binderOptions,
  eagerly: Boolean): FakeApplicationBuilder =
